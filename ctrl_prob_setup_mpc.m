function solver = ctrl_prob_setup_mpc(lin_sys,ctrl)
   ...
   P = blkdiag( kron(speye(ctrl.N), ctrl.Q), ctrl.QT, kron(speye(ctrl.N), ctrl.R) );
   q = [repmat(-ctrl.Q*zeros(ctrl.nx,1), ctrl.N,1); -ctrl.QT*zeros(ctrl.nx,1);zeros(ctrl.N*ctrl.nu, 1)];
   ...
   Aeq = [Ax, Bu];
   leq = [-zeros(ctrl.nx,1); zeros(ctrl.N*ctrl.nx, 1)];
   ueq = leq;
